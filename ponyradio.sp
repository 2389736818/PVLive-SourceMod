...
{
    LoadTranslations("ponyradio.phrases");
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && !IsFakeClient(i) && tunedstation[i] == arrayid)
		{
			...
			} else
			{
				...
				if (chance > GetConVarInt(g_Adverts)) CPrintToChat(i, "%t", "Tune in");
			}
		}
	}
}
...
public Action:DisplayStations(client, args)
{
	...
	if (args > 0) {
		...
		for (new i = 0; i < size; i++) {
			...
			if (StrContains(station, arg1, false) != -1 && stationid == -1) stationid = i;
				...
				CPrintToChat(client,"%t", "More than one radio station matches that name");
				break;
			}
		}

		if (stationid == -1) {
			CPrintToChat(client,"%t", "No radio station matches that name.");
		}
		else if (stationid != -1) {
	...
	if (stationid <= 0) {
		...
		for (new i = 0; i < size; i++) {
			...
			//	馃憘 喋? 喹?釥€ 岌?鈽?鈾?  Yeah....just try and guess what i was doing here
			Format(buffer, sizeof(buffer), "%s  鈽?d", station, GetArrayCell(A_Listeners,i)); 
...
public Action:RadioAll(client, args)
{
	if(tunedstation[client] != -1)
	{
		for(new i=1; i < MAXPLAYERS; i++) {
			if (reconnect[i] > 2) {
				CPrintToChat(i,"%t", "!radiooff !vol !np");
		...
		CPrintToChat(client,"%t", "Listen to a radio yourself first");	//yay for laziness
	}
}
...
public Action:SetVolume(client, args)
{
	...
	{	
		...
		if (old == volume){
			CPrintToChat(client, "%t", "Volume is Already set to", volume);
		} else {
			if(tunedstation[client] != -1) PlayerOTD(client);
			CPrintToChat(client, "%t", "Volume set to", volume);
		}
	}
}
...
public OutputSongs(client, id)	//array id
{
	...
	if (id == -1)
	{
		for (new i = 0; i < size; i++) { 					//if they arnt listening to a station, 
			...
			CPrintToChat(client, "%t", "playing", station, artist, song);
		}	
	} else {
		...
		CPrintToChat(client, "%t", "playingtwo", station, artist, song);
	}
	return;
}
...
{
	CPrintToChat(client, "%t", "Make sure you have flash installed for OTHER BROWSERS");
}
